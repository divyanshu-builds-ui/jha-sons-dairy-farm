...
export function generateRegisterPDF({ retailers, products, orders, area, date }) {
  ...
  retailers.forEach((ret, i) => {
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(7.5);
    doc.text((ret.name || '-').slice(0, 18), m + numW + 2, y + retailerRowH / 2 + 0.5);
    doc.setTextColor(0, 0, 0);
  ...
  if (otherItems.length > 0) {
    ...
    otherItems.forEach((oi, idx) => {
      ...
      doc.setFont('helvetica', 'bold');
      doc.text(oi.retailer.slice(0, 22), m + 10, y + 5);
      doc.setFont('helvetica', 'normal');
...
export function generateChalanPDF({ retailers, orders, area, date }) {
  ...
  retailerPhones.forEach((phone) => {
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(8.5);
    doc.text(ret.name || retOrders[0]?.retailer || '-', x + 3, y + 2);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(6.5);
    ...
    items.slice(0, maxItems).forEach((item, i) => {
      ...
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(6.5);
      doc.text(item.name.slice(0, 20), cx + 1, y + 3.5); cx += cols.name;
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(6);
...
export function generateDeliveryPDFLegacy(orders, area, date) {
  ...
  orders.forEach((order, i) => {
    ...
    doc.setFontSize(9);
    doc.setTextColor(0, 0, 0);
    doc.text(`${i + 1}. ${order.retailer}  (${order.phone || ''})`, 10, y + 4);
