...
const NAV = [
  ...
  { to: '/milk',     label: 'Our Milk' },
  { to: '/schemes',  label: 'Schemes' },
  { to: '/gau-seva', label: 'Gau Seva' },
...
function Navbar() {
  ...
  return (
    <header style={{ top: '32px' }} className={`fixed inset-x-0 z-50 transition-all duration-300 ${
      scrolled
        ? 'bg-white/96 backdrop-blur-md shadow-sm border-b border-sand-200'
        : 'bg-transparent'
    }`}>
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <Link to="/" className="flex items-center gap-2.5 shrink-0">
            ...
            <div className="leading-tight">
              <p className="font-black text-sm text-sand-950 tracking-tight">{BRAND.shortName}</p>
              <p className="text-2xs text-sand-500 font-medium">Dairy Farm · {BRAND.city}</p>
            </div>
          </Link>
          ...
          <div className="flex items-center gap-2">
            <a href={`https://wa.me/${BRAND.whatsapp}?text=Hi, I want to start milk delivery`}
              ...
              className="hidden sm:flex btn-whatsapp text-sm py-2 px-4 gap-1.5">
              <WhatsAppIcon size={15} /> WhatsApp
            </a>
            <a href={`tel:${BRAND.phone}`}
              className="hidden sm:flex btn-ghost border border-sand-200 text-sm py-2 px-3">
              <Phone size={13} /> Call
            </a>
            <button onClick={() => setOpen(v => !v)}
              className="md:hidden w-9 h-9 flex items-center justify-center rounded-lg border border-sand-200 text-sand-600 hover:bg-sand-100 transition-colors">
            ...
            <div className="container-site py-3 space-y-0.5">
              ...
              <div className="pt-2 pb-1 grid grid-cols-2 gap-2">
                <a href={`https://wa.me/${BRAND.whatsapp}`} target="_blank" rel="noopener noreferrer"
                  className="btn-whatsapp justify-center py-2.5">
                  <WhatsAppIcon size={15} /> WhatsApp
                </a>
                <a href={`tel:${BRAND.phone}`} className="btn-outline justify-center py-2.5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 mb-8">
          ...
          <div>
            <div className="flex items-center gap-2.5 mb-3">
              ...
              <div>
                ...
                <p className="text-2xs text-sand-600">Dairy Farm</p>
              </div>
            </div>
          ...
          <div>
            ...
            <div className="space-y-1.5">
              {NAV.map(({ to, label }) => (
                <Link key={to} to={to}
                  className="block text-sm text-sand-500 hover:text-white transition-colors">
                  {label}
                </Link>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="space-y-2.5 text-sm">
              ...
              <p className="text-sand-500 leading-relaxed text-xs">{BRAND.address}</p>
            </div>
          </div>
        </div>
...
export default function PublicLayout() {
  return (
    <div className="min-h-screen bg-sand-50">
      <Navbar />
      <main style={{ paddingTop: 'calc(4rem + 32px)' }}>
        <AnimatedOutlet />
      </main>
      <Footer />
    </div>
